...
| `RITE_VERSION` | string | rite's version (e.g. `v1.0.6`, or the local-fallback from `internal/version/version.txt`) |
...
rite's version string. During a release build this is injected by goreleaser (`v1.0.6`); for local `go install` builds it falls back to `internal/version/version.txt` (which tracks the last shipped tag). Handy for gating behavior on rite version or stamping it into build output:
