...
**Status: v1.0.4 shipped.** See [`CHANGELOG.md`](./CHANGELOG.md) for per-release notes and [`RELEASING.md`](./RELEASING.md) for the cut procedure.
...
```
Downloads the latest release archive, verifies its SHA-256 against `rite_checksums.txt`, and drops `rite` into `~/bin`. Pass a tag as the last argument to pin a version (`… | sh -s -- -b ~/bin v1.0.4`). Default bindir is `./bin` if `-b` is omitted.

**mise**:
```toml
# mise.toml
[tools]
"ubi:clintmod/rite" = "v1.0.4"
```
