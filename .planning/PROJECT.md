...
- **Core library lives at `internal/task/` (package `task`).** The library code used to sit at the module root; it was moved under `internal/task/` so the repo root only holds `cmd/`, `internal/`, `taskfile/`, and config. External callers import `task "github.com/clintmod/rite/internal/task"`. Embedded assets (`completion/`, `templates/default.yml`) and `testdata/` moved with the package since `//go:embed` and Go test resolution are package-dir-scoped.
