...
- **Docs site:** `website/` is a pure VitePress project; `.github/workflows/pages.yml` builds on every push that touches `website/**` and publishes to `clintmod.github.io/rite/` via GitHub Pages (Source: GitHub Actions — enabled via `gh api POST /repos/clintmod/rite/pages`). Custom domain can be added later by dropping `DOCS_BASE=/rite/` from `pages.yml`.
  - **Versioned layout (since #132):** `website/src/next/` holds the bleeding-edge main-branch docs; each released tag has an immutable snapshot under `website/src/v<X.Y.Z>/`. The VitePress config (`website/.vitepress/config.ts`) scans these dirs at build time and generates per-version sidebars + a **Version** dropdown in the nav. `/` serves a client-side redirect to the latest released version; `/next/` is the explicit path for unreleased docs. Internal markdown links inside a version dir must be **relative** (`./foo` not `/foo`) so they resolve within-version. The canonical schema still lives at `website/src/public/schema.json` + `schema/v3.json` (stable consumer URL `clintmod.github.io/rite/schema.json`); `generate:schema` mirrors it into `website/src/next/public/` too.
  - **Snapshotting new releases:** `rite release:snapshot-docs VERSION=X.Y.Z` copies `next/` into `v<VERSION>/` with link rewrites + spec/changelog stubs. It's invoked automatically from `release:prepare` so the staging PR already carries the snapshot. `spec.md` / `changelog.md` in versioned dirs are stubs that link to the tag-pinned GitHub blobs — inlining the real SPEC/CHANGELOG crashes the Vue SFC compiler on Go-template syntax inside fenced blocks (see "Known sharp edges" below).
