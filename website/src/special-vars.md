...
| `RITE_VERSION` | string | rite's version (e.g. `v2026.4.14`, or the local-fallback from `internal/version/version.txt`) |
...
rite's version string. During a release build this is injected by goreleaser (`v2026.4.14`); for local `go install` builds it falls back to `internal/version/version.txt` (which tracks the last shipped tag). Handy for gating behavior on rite version or stamping it into build output:
